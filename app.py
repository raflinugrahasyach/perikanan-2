...
try:
    ...
    # (Placeholder configs... )
    # PATH INI SEHARUSNYA DIGANTI OLEH file classification_config.py
...
#mengkompres gambar
def compress_image(file_path, output_path, max_size_mb=20, quality=85):
    """
    Mengompresi gambar agar ukurannya tidak melebihi batas maksimal.
    Args:
        file_path (str): Path file asli.
        output_path (str): Path file hasil kompresi.
        max_size_mb (int): Ukuran maksimal dalam MB.
        quality (int): Kualitas JPEG (1-100).
    """
    with Image.open(file_path) as img:
        ...
        while os.path.getsize(output_path) > max_size_mb * 1024 * 1024:
            quality -= 5  # Kurangi kualitas jika ukuran masih terlalu besar
            img.save(output_path, format="JPEG", quality=quality, optimize=True)

        print(f"Gambar berhasil dikompresi: {output_path} (Ukuran: {os.path.getsize(output_path) / (1024 * 1024):.2f} MB)")
...
# --- (Fungsi helper /analysis, /dashboard, /market TETAP SAMA) ---
# ... (Saya sembunyikan agar ringkas, JANGAN HAPUS DARI FILE ANDA JIKA SUDAH ADA) ...
...
def classification():
    ...
    def compress_image(image, max_size_mb=2, quality=85):
        buf = BytesIO()
        image.save(buf, format="JPEG", quality=quality, optimize=True)

        while len(buf.getvalue()) > max_size_mb * 1024 * 1024:
            buf = BytesIO()
            quality -= 5
            if quality < 20:
                break
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if 'file' in request.files and request.files['file'].filename != '':
                ...
                image = compress_image(image, max_size_mb=2, quality=85)

            # ---------------------------
            # 2. INPUT DARI WEBCAM (BASE64)
            # ---------------------------
                ...
                image = image.convert("RGB")
                image = compress_image(image, max_size_mb=2, quality=85)
                print("Gambar webcam dikompresi sebelum klasifikasi")
            ...
            # ---------------------------
            # 3. KONVERSI KE BASE64 UNTUK DITAMPILKAN DI HTML
            # ---------------------------
            buf = BytesIO()
