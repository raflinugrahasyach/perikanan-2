...
#mengkompres gambar
def compress_image(file_path, output_path, max_size_mb=2, quality=85):
    """
...
def classification():
    ...
    def compress_image(image, max_size_mb=2, quality=85):
        ...
        Mengompresi gambar agar ukurannya tidak melebihi batas maksimal.
        Args:
            image (PIL.Image): Objek gambar PIL.
            max_size_mb (int): Ukuran maksimal dalam MB.
            quality (int): Kualitas JPEG (1-100).
        Returns:
            PIL.Image: Gambar yang sudah dikompresi.
        """
        buf = BytesIO()
        image.save(buf, format="JPEG", quality=quality, optimize=True)
        
        while len(buf.getvalue()) > max_size_mb * 1024 * 1024:
            buf = BytesIO()  # Reset buffer
